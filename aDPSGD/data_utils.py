...
def load_data(data_type, data_options, replace_index=None, data_privacy='all'):
    ...
    if data_type == 'mnist':
        ...
        if data_options['preprocessing'] == 'PCA':
            ...
            pca = False
        x_train, y_train, x_test, y_test = load_adult(data_privacy=data_privacy,
                pca=pca)
    else:
...
def validation_split(x_train, y_train, x_test, y_test):
    ...
    if replace_index:
        ...
        # and then DON'T USE THE FIRST ROW (this is to avoid an effect where experiments where the replace_index is low encounter an unusually
...
def load_protein(data_privacy='all'):
    ...
    try:
        ...
        # now normalise
        # they say they normalise to the unit sphere (so not normalising anything above that value to that value, everything)
...
def load_housing(binary=False, enforce_max_norm=True, data_privacy='all'):
    ...
    # normalise
    # this is kinda problematic given i split into vali after this
...
#    x_test = scale*x_test/16
#    sns.distplot(np.linalg.norm(x_train, axis=1), label='train')
#    sns.distplot(np.linalg.norm(x_test, axis=1), label='test')
#    plt.savefig('x_scaled_' + str(scale) + '.png')
#    plt.clf()
   #x_train = 0.5*x_train
   #x_test = 0.5*x_test
    return x_train, y_train, x_test, y_test
...
def load_adult(data_privacy='all', pca=False):
    ...
    try:
        ...
        x_train_orig = x_train.copy()
        # do public/private split
        x_train, y_train, x_test, y_test = public_private_split('adult', data_privacy, x_train, y_train, x_test, y_test)
