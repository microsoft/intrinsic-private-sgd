...
def bivariate_gradients(dataset, model, replace_index, seed, df=None,
                        params=['#3', '#5'], iter_range=(None, None), n_times=2, save=False):
...
def fit_pval_histogram(what, dataset, model, t, n_experiments=3, diffinit=False,
    ...
    for i, replace_index in enumerate(replace_indices):
        ...
        for j, p in enumerate(params):
            ...
            df_fit = derived_results.estimate_statistics_through_training(what=what, dataset=None,
...
def visualise_fits(dataset, model, replace_index, seed, save=True, params=None):
    ...
    for i, p in enumerate(params):
        ...
        df_fit = derived_results.estimate_statistics_through_training(what='gradients', dataset=dataset,
                                                                      model=model, replace_index=replace_index,
                                                                      seed=seed, params=[p])
...
def visualise_trace(datasets, models, replaces, seeds, privacys, save=True,
    ...
    if include_convergence:
        for j, identifier in enumerate(identifiers):
            dataset, model, replace_index, seed, data_privacy = identifier
            convergence_point = derived_results.find_convergence_point_for_single_experiment(dataset, model,
                                                                                             replace_index,
                                                                                             seed, diffinit,
...
def examine_parameter_level_gradient_noise(dataset, model, replace_index, seed,
                                           times=[10, 25], save=True, params=['#1', '#5']):
...
def visually_compare_distributions(dataset, model, replace_index, seed, df=None,
                                   times=[10, 25], save=False, iter_range=(None, None), params=None):
...
def visualise_weight_trajectory(dataset, identifiers, df=None, save=True,
                                ...
                                include_optimum=False, include_autocorrelation=False, diffinit=False):
    """
    """
    df_list = []
