...
def get_available_results(dataset, model, replace_index=None, seed=None, diffinit=False, data_privacy='all'):
    ...
    # remove replace_NA!
    available_results = [x for x in available_results if 'replace_NA' not in x]
...
def get_posterior_samples(dataset, iter_range, model='linear', replace_index=None,
    ...
    for i, s in enumerate(available_seeds):
        weights_from_s = load_weights(dataset, model, replace_index=replace_index,
                                      seed=s, iter_range=iter_range, params=params,
                                      verbose=False, diffinit=diffinit, data_privacy=data_privacy)
        try:
...
def load_gradients(dataset, model, replace_index, seed, noise=False,
                   iter_range=(None, None), params=None, verbose=False, diffinit=False):
...
def load_loss(dataset, model, replace_index, seed, iter_range=(None, None),
              diffinit=False, verbose=False, data_privacy='all'):
    path_stub = trace_path_stub(dataset, model, replace_index, seed, diffinit=diffinit,
