...
def plot_delta_histogram(dataset, model, num_deltas='max', t=500, include_bounds=False, xlim=None, ylim=None, data_privacy='all', plot=True, multivariate=False):
    """
...
def plot_epsilon_distribution(dataset, model, t, delta, n_pairs, 
    ...
    if variability_from == 'local':
        ...
        variability = derived_results.estimate_variability(dataset, model, t, multivariate=False, diffinit=False)
        variability_diffinit = derived_results.estimate_variability(dataset, model, t, multivariate=False, diffinit=True)
...
def plot_sens_and_var_over_time(dataset, model, num_deltas=500, iter_range=(0, 1000), 
    ...
    axarr[0].fill_between(df['t'], train_loss - train_loss_std, train_loss + train_loss_std, label='_nolegend_', alpha=0.2, color='black')
    ...
    axarr[0].fill_between(df['t'], vali_loss - vali_loss_std, vali_loss + vali_loss_std, label='_nolegend_', alpha=0.2, color='grey')
    ...
    if model == 'logistic':
        convergence_points = experiment_metadata.lr_convergence_points
        title = experiment_metadata.dataset_names[dataset] + ' (logistic regression)'
    else:
        convergence_points = experiment_metadata.nn_convergence_points
        title = experiment_metadata.dataset_names[dataset] + ' (neural network)'
...
#    sigma_v_seed = sigma_df[sigma_df['n_replaces'] == sigma_df['n_replaces'].max()][['n_seeds', 'sigma']]
    ...
    sigma_we_use = derived_results.estimate_variability(dataset, model, t, diffinit=True)
    ...
    axarr.set_title(experiment_metadata.dataset_names[dataset] + ' (' + experiment_metadata.model_names[model] + ')')
    ...
    sens_we_use = derived_results.estimate_sensitivity_empirically(dataset, model, t,
            num_deltas='max', diffinit=True, data_privacy='all')
...
def plot_sigmas_distribution(model, datasets=None, ylim=None):
    ...
    for ds in datasets:
        ...
        sns.distplot(all_sigmas - min_sigma, ax=axarr, norm_hist=True, label=experiment_metadata.dataset_names[ds], color=to_rgba(experiment_metadata.dataset_colours[ds]), kde=False, bins=50)
...
def compute_sigma_v_n_seeds(dataset, model, t):
    ...
    for n_seeds in [2, 5, 10]*5 + [20, 30]*3 + [40, 50]*2 + [60, 70, 80, 90, 100, 200]:
        for n_replaces in [25]:
            ...
            sigma = derived_results.estimate_variability(dataset, model, t=t, 
                    n_seeds=n_seeds, n_replaces=n_replaces, ephemeral=True, diffinit=True)
...
def compute_sens_v_n_deltas(dataset, model, t):
    ...
    for n_deltas in [5, 10, 25, 50, 75, 100, 125, 250, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000]:
        vary_S, _ = derived_results.get_deltas(dataset, iter_range=(t, t+1), 
...
def compare_mnist_variants():
    ...
    sns.distplot(mnist_binary_pca.flatten(), ax=axarr, label='PCA', norm_hist=True, kde=False, color=to_rgba(colours[0]))
    sns.distplot(mnist_binary_grp.flatten(), ax=axarr, label='GRP', norm_hist=True, kde=False, color=to_rgba(colours[1]))
    sns.distplot(mnist_binary_cropped.flatten(), ax=axarr, label='Crop', norm_hist=True, kde=False, color=to_rgba(colours[2]))
    ...
    plt.close()
    # ------- compare the traces ----- #
    pca_loss = eval_utils.aggregated_loss('mnist_binary_pca', 'logistic', diffinit=True)
    grp_loss = eval_utils.aggregated_loss('mnist_binary', 'logistic', diffinit=True)
    cropped_loss = eval_utils.aggregated_loss('mnist_binary_cropped', 'logistic', diffinit=True)
    ...
    axarr[0].scatter(cropped_loss.index, cropped_loss['binary_crossentropy_mean_train'], label='Crop', s=size, color=colours[2])
    axarr[0].plot(pca_loss.index, pca_loss['binary_crossentropy_mean_train'], label='_nolegend_', color=colours[0], alpha=0.5)
    axarr[0].plot(grp_loss.index, grp_loss['binary_crossentropy_mean_train'], label='_nolegend_', color=colours[1], alpha=0.5)
    axarr[0].plot(cropped_loss.index, cropped_loss['binary_crossentropy_mean_train'], label='_nolegend_', color=colours[2], alpha=0.5)
    ...
    axarr[1].scatter(cropped_loss.index, cropped_loss['binary_accuracy_mean_train'], label='Crop', s=size, color=colours[2])
    axarr[1].plot(pca_loss.index, pca_loss['binary_accuracy_mean_train'], label='_nolegend_', color=colours[0], alpha=0.5)
    axarr[1].plot(grp_loss.index, grp_loss['binary_accuracy_mean_train'], label='_nolegend_', color=colours[1], alpha=0.5)
    axarr[1].plot(cropped_loss.index, cropped_loss['binary_accuracy_mean_train'], label='_nolegend_', color=colours[2], alpha=0.5)
...
def overlay_pval_plot(model='logistic', xlim=None, n_experiments=50,
    ...
    for ds in datasets:
        ...
        log_pvals, n_params = vis_utils.fit_pval_histogram(what=what, dataset=ds, model=model,
                t=convergence_points[ds], n_experiments=n_experiments, plot=False)
        sns.distplot(log_pvals, kde=True, bins=min(100, int(len(log_pvals)*0.25)),
                ax=axarr, color=dataset_colours[ds], norm_hist=True, label=experiment_metadata.dataset_names[ds],
                kde_kws={'alpha': 0.6})
        if not n_params in vertical_lines_we_already_have:
            axarr.axvline(x=np.log(0.05/(n_params*n_experiments)), ls='--', label='p = 0.05/' + str(n_params*n_experiments),
                    color=dataset_colours[ds], alpha=0.75)
...
def overlay_eps_plot(model='logistic', datasets=None, xlim=None, ylim=None, title=None):
    ...
    if model == 'logistic':
        ...
        convergence_points = experiment_metadata.nn_convergence_points
    if datasets is None:
        datasets = experiment_metadata.dataset_colours.keys()
        plot_label = '_'
    else:
        plot_label = '_'.join(datasets) + '_'
    fig, axarr = plt.subplots(nrows=1, ncols=1, figsize=figsize)
    for ds in datasets:
        _, eps_diffinit = derived_results.epsilon_distribution(ds, model, t=convergence_points[ds], 
                delta=None, n_pairs=None, which='vary', sensitivity_from='local', 
                sharex=True, variability_from='empirical', xlim=None, ylim=None,
                data_privacy='all', plot=False)
        sns.distplot(eps_diffinit, ax=axarr, 
                label=experiment_metadata.dataset_names[ds],
                color=experiment_metadata.dataset_colours[ds],
                bins=n_bins,
